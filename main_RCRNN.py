...
from utils.settings_RCRNN import *
...
def main(iteration=None):
    ...
    logger.info("batch : " + str(server_cfg["batch_size"]))  # 추가
...
def train(train_cfg):
    ...
    for _, (wavs, labels, _, _) in enumerate(tk0, 0):
        ...
        loss_class_strong = train_cfg["criterion_class"](strong_pred_stud[mask_strong],  ##### time-level
                                                         labels[mask_strong])
        loss_class_weak = train_cfg["criterion_class"](weak_pred_stud[mask_weak], labels_weak)  ##### clip-level

        # consistency losses
        loss_cons_strong = train_cfg["criterion_cons"](strong_pred_stud, strong_pred_tch.detach())   #### time
        loss_cons_weak = train_cfg["criterion_cons"](weak_pred_stud, weak_pred_tch.detach())  #### clip
    ...
    with torch.no_grad():
        ...
        for _, (wavs, labels, _, indexes, filenames, paths) in enumerate(tk1, 0):
            ...
            if torch.any(mask_weak):
                labels_weak = (torch.sum(labels[mask_weak], -1) > 0).long()  # labels_weak size = [bs, n_class]
