...
from utils.model_CRNN_baseline import *
from utils.dataset import *

import sys

# sys.path.append('Data2/dcase_2022_T4/FDY-SED_nr/panns/')
...
def get_mt_datasets(configs, server_cfg, train_cfg):
    ...
    if train_cfg["strong_real"]:
        strong_dir = dataset_cfg["strong_folder"]    #추가
        strong_df = pd.read_csv(dataset_cfg["strong_tsv"], sep="\t") #추가
        
        strong_train_dataset = StronglyLabeledDataset(strong_df, strong_dir, False, encoder) #추가
...
#     strong_train_dataset = StronglyLabeledDataset(strong_df, strong_dir, False, encoder) #추가
    ...
    if train_cfg["strong_real"]:
        strong_full_set = torch.utils.data.ConcatDataset([strong_train_dataset, synth_train_dataset])  #추가
        train_data = [strong_full_set, weak_train_dataset, unlabeled_dataset]
    else:
...
def get_w2v2_datasets(configs, server_cfg, train_cfg):
    general_cfg = configs["generals"]
    encoder = train_cfg["encoder"]
    dataset_cfg = configs["dataset"]
    w2v2_cfg = configs["wav2vec2"]
    batch_size_val = server_cfg["batch_size_val"]
    num_workers = server_cfg["num_workers"]
    batch_sizes = server_cfg["batch_size"]
    synthdataset_cfg = configs["synth_dataset"]
    ...
    if train_cfg["strong_real"]:
        strong_dir = w2v2_cfg["strong_w2v2"]    # strong w2v2 feature path
        strong_df = pd.read_csv(dataset_cfg["strong_tsv"], sep="\t") 
        
        strong_train_dataset = StronglyLabeledDataset(strong_df, strong_dir, False, encoder) 
    
    
    synth_train_dir = w2v2_cfg["synth_train_w2v2"]  #synth train w2v2 feature path
    synth_train_tsv = synthdataset_cfg["synth_train_tsv"]
    synth_train_df = pd.read_csv(synth_train_tsv, sep="\t")
    weak_dir = w2v2_cfg["weak_w2v2"]   #weak w2v2 feature path
    weak_df = pd.read_csv(dataset_cfg["weak_tsv"], sep="\t")
    weak_train_df = weak_df.sample(frac=train_cfg["weak_split"], random_state=train_cfg["seed"])
    weak_valid_df = weak_df.drop(weak_train_df.index).reset_index(drop=True)
    weak_train_df = weak_train_df.reset_index(drop=True)
    unlabeled_dir = w2v2_cfg["unlabeled_w2v2"] #unlabeled w2v2 feature path
    synth_valid_dir = w2v2_cfg["synth_valid_w2v2"]  #synth valid w2v2 feature path
    synth_valid_tsv = synthdataset_cfg["synth_val_tsv"]
    synth_valid_df = pd.read_csv(synth_valid_tsv, sep="\t")
    synth_valid_dur = synthdataset_cfg["synth_val_dur"]

#     strong_train_dataset = StronglyLabeledDataset(strong_df, strong_dir, False, encoder) #추가
    synth_train_dataset = StronglyLabeledDataset(synth_train_df, synth_train_dir, False, encoder)
    weak_train_dataset = WeaklyLabeledDataset(weak_train_df, weak_dir, False, encoder)
    unlabeled_dataset = UnlabeledDataset(unlabeled_dir, False, encoder)
    synth_vaild_dataset = StronglyLabeledDataset(synth_valid_df, synth_valid_dir, True, encoder)
    weak_valid_dataset = WeaklyLabeledDataset(weak_valid_df, weak_dir, True, encoder)
    
    
    if not general_cfg["test_on_public_eval"]:
        test_dir = w2v2_cfg["test_w2v2"]  # tset w2v2 feature path
        test_tsv = dataset_cfg["test_tsv"]
        test_df = pd.read_csv(test_tsv, sep="\t")
        test_dur = dataset_cfg["test_dur"]
        test_dataset = StronglyLabeledDataset(test_df, test_dir, True, encoder)
    else:
        test_tsv = dataset_cfg["pubeval_tsv"]
        test_df = pd.read_csv(test_tsv, sep="\t")
        test_dur = dataset_cfg["pubeval_dur"]
...
# def get_pretrained_models(configs, train_cfg, multigpu):
#     net = CRNN(**configs["CRNN"])
...
def get_f1calcs(n_class, device):
    stud_f1calc = torchmetrics.F1Score(n_class, average="macro", multilabel=True, compute_on_step=False)
    tch_f1calc = torchmetrics.F1Score(n_class, average="macro", multilabel=True, compute_on_step=False)
